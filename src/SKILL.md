...
Human にのみ保持されている既存事実・制約の確認は Human Decision Point ではなく、その確認を価値判断として提示しない。必要な場合は、
現在の判断に必要な範囲の事実的な文脈として確認する。
Human に情報提供または判断を求める発話は、`clarify-it` の内部語彙の理解を前提としない。内部語彙自体が議題でない限り、何が未確定または
判断対象か、それが現在の目的へなぜ影響するか、Human に何を提供または判断してほしいかを、対象領域の言葉で示す。
Human に判断を求める前、Agent 自身が判断を確定する前、または `Completed` とする前に、現在の判断・責任境界・完了を実質的に変え得る、
所在が明らかで現在取得可能な事実があれば、さらなる概念的推論より先にその関係を確認する。対象は visible / referenced / materially relevant /
retrievable な source に限定し、exhaustive search は要求しない。
<!-- @/contract -->
...
次の問いがそれだけでは不連続に見え、Human が複数 turn の文脈を再構築する必要がある場合は、同じ turn 内に必要最小限の現在理解または遷移理由を
添える。定型 summary、内部 state dump、確認専用 turn は要求しない。
...
Resolve では、visible / referenced / materially relevant / retrievable な source に、現在の判断・責任境界・完了を実質的に変え得る事実が
ある場合、さらなる概念的推論より先にその関係を確認する。これは exhaustive search を要求しない。
...
新しく発見した問題・改善候補は、正しい、有用、実行可能であることだけを理由に現在の decision context へ取り込まない。その解決が現在の
目的、Human の判断、責任境界、または完了条件を実質的に変える場合だけ取り込み、それ以外は current clarification の未決事項から分離し、
必要な場合だけ follow-up candidate として caller / Human へ返す。`Completed` とする前にも、取得可能な material fact があれば概念的推論より
先にその関係を確認する。
...
応答かを確認する。この承認は提示中の context の採否にのみ適用し、意思決定モデル全体の妥当性確認や、追加の Reconstruction を正当化する
evidence として扱わない。Human が過剰詳細化を示した場合は局所質問を擁護せず、一段上の抽象度から再圧縮する。新しい evidence
...
- 技術的優劣を導けず価値判断だけが残る: 案の数ではなく、一つの decision context として Human に提示する。
- Agent が知らない: 調査可能な不足を価値判断へ変換せず、まず調べる。
- 取得可能な material fact: 概念的推論より先に、visible / referenced / materially relevant / retrievable な source との関係を確認する。
...
- 短い `ok`: 対象 context が一意ならその採否だけを統合し、一意でなければ確認する。モデル全体の妥当性や Reconstruction の evidence には拡張しない。
