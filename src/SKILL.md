...
実質的に変え得る visible / referenced / materially relevant / retrievable な source が現在取得可能なら、exhaustive search はせず、
...
新しい問題・改善候補は、目的・Human の判断・責任境界・完了条件を実質的に変える場合だけ current context に取り込み、それ以外は未決事項と
完了条件から分離し、必要な場合だけ caller / Human へ follow-up candidate として返す。

短い承認は提示中の一意な context の採否に限り、モデル全体や Reconstruction の妥当性へ拡張しない。

Human Decision Point などの内部の構造・判定用語は、意思決定モデルと caller / projection の判定用語として維持し、意味を rename / redefine しない。
内部語彙は内部判定用語として維持するが、内部語彙自体が議題でない限り Human に理解を要求しない。情報提供または判断を求める発話では、
...
短い `ok` は、応答対象の context が一意な場合だけその採否として統合し、何への応答か不明なら確認する。モデル全体の妥当性や Reconstruction の
evidence には拡張しない。Human が過剰詳細化を示した場合は局所質問を擁護せず、一段上の抽象度から再圧縮する。新しい evidence
